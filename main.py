...
def generate_product_json(data):
    try:
        product = normalize_input(data["product_name"])
        brand = normalize_input(data["brand"])
        sku = normalize_input(data["sku"])

        config = {
            "tone": data.get("tone", "Write professionally and highlight key features."),
            "temperature" : safe_float(data.get('temperature'), 0.7),
            "top_p" : safe_float(data.get('top_p'), 1.0),
            "max_tokens" : int(data.get('max_tokens')) if data.get('max_tokens') else 800,
            "short_limit": int(data.get("short_desc_words")) if data.get("short_desc_words") else 25,
            "long_limit": int(data.get("long_desc_words")) if data.get("long_desc_words") else 200,
